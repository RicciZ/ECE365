...
def prune_vocabulary(training_counts, target_data, min_counts):
    ...
    '''
    pruned_vocab = [w for w,c in training_counts.items() if c >= min_counts]
    counter_list = [Counter({w:c for w,c in x.items() if w in pruned_vocab}) for x in target_data]
    return counter_list,pruned_vocab
...
def make_feature_vector(base_features,label):
    ...
    '''
    f = {}
    for w in base_features:
        f[(label,w)] = base_features[w]
    f[(label,OFFSET)] = 1
    return f
...
def predict_all(x,weights,labels):
    ...
    '''
    return np.array([predict(counter,weights,labels)[0] for counter in x])
...
def get_corpus_counts(x,y,label):
    ...
    """
    return defaultdict(float,sum([x[i] for i in range(len(y)) if y[i] == label],Counter()))
...
def estimate_pxy(x,y,label,smoothing,vocab):
    ...
    tot = sum(counts.values()) + (smoothing*len(vocab))
    return defaultdict(float,{w:np.log((counts[w]+smoothing)/tot) for w in vocab})
...
def estimate_nb(x,y,smoothing):
    ...
    """
    scores = {}
    vocab = [w for w,c in sum(x,Counter()).items()]
    labels = set(y)
    f = defaultdict(float)
    for label in labels:
        p_dict = estimate_pxy(x,y,label,smoothing,vocab)
        for w,p in p_dict.items():
            f[(label,w)] = p
        f[(label,OFFSET)] = np.log(len(y[label==y])/len(y))
    return f
...
def find_best_smoother(x_tr_pruned,y_tr,x_dv_pruned,y_dv,smoothers):
    ...
    smoothers_val = {}
    labels = set(y_tr)
    for smt in smoothers:
        weights = estimate_nb(x_tr_pruned,y_tr,smt)
        y_hat = predict_all(x_dv_pruned,weights,labels)
        smoothers_val[smt] = acc(y_hat,y_dv)
    return smoothers_val[min(smoothers_val,key=lambda k: smoothers_val[k])],smoothers_val
...
def make_numpy(bags_of_words, vocab):
    ...
    :rtype: numpy array
    '''
    nparray = np.zeros((len(bags_of_words),len(vocab)))
    vocab = sorted(vocab)
    for i in range(len(bags_of_words)):
        for j in range(len(vocab)):
            if vocab[j] in bags_of_words[i]:
                nparray[i,j] = bags_of_words[i][vocab[j]]
    return nparray

# deliverable 4.2
def better_model():
    scikit_log_reg = LogisticRegression(max_iter=1000,C=0.01)   ## Tune parameters for this function.
    ### BEGIN SOLUTION
    ### END SOLUTION
...
def get_top_features_LR(scikit_log_reg, vocab,label_set,label,k):
    ...
    vocab=sorted(vocab)
    V = [w for _,w in sorted(zip(coef[label_set.index(label)],vocab))]
    most_indicative_features = V[-k:]
    least_indicative_features = V[:k]
    ### END SOLUTION
...
def get_top_features_NB(theta_nb, label_set,label,k):
    ...
    for l,w in theta_nb:
        if l == label:
            f[w] = theta_nb[(l,w)]
    del f[OFFSET]
    V = [w for w,_ in sorted(f.items(),key=lambda i: i[1])]
    most_indicative_features = V[-k:]
    least_indicative_features = V[:k]
    ### END SOLUTION
    return most_indicative_features,least_indicative_features
...
def get_PRF(Y_hat_dv, Y_dv, label_set, label):
    ...
    ### BEGIN SOLUTION
    label_id = label_set.index(label)
    tp = sum((Y_hat_dv==label_id) & (Y_dv==label_id))
    fp = sum((Y_hat_dv==label_id) & (Y_dv!=label_id))
    fn = sum((Y_hat_dv!=label_id) & (Y_dv==label_id))
    precision = tp/(tp+fp)
    recall = tp/(tp+fn)
